...
export class News extends Component {
  ...
  handleNextClk = () => {
    if (this.state.page < Math.ceil(this.state.articles.length / 10)) {
      this.setState((prevState) => ({ page: prevState.page + 1 }));
    }
  };
    ...
    return (
      <div className="container my-5 ">
        ...
        <div className="container d-flex justify-content-between">
          <button type="button" class="btn btn-danger" onClick={this.handlePreClk}>
            &larr; Previous
          </button>
          <button type="button" class="btn btn-danger" onClick={handleNextClk}>
            next &rarr;
          </button>
        </div>
      </div>
